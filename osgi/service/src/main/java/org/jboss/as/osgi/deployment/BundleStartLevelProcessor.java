...
public class BundleStartLevelProcessor implements DeploymentUnitProcessor {
    ...
    private Integer getDeploymentStartLevel(DeploymentUnit depUnit) {
        DeploymentDependencies metadata = BundleDeploymentProcessor.getDeploymentMetadata(depUnit);
        String level = metadata.getProperty(ClientConstants.DEPLOYMENT_METADATA_STARTLEVEL);
        if (level == null) {
            ...
            if (slAware != null) {
                ...
                level = value != null ? value.asString() : null;
            }
        }
        if (level == null) {
